...
import AddLandingPageCategoriesModal from "./AddCategoriesModal";

export default function LandingPageCategories({ categories, refetch, className }) {
    const [modal, setModal] = useState();
    const [loading, setLoading] = useState();
    const deleteMeal = async (id) => {
        if (!id || !window.confirm("هل أنت متأكد من حذف الوجبة؟")) return;
        try {
            const res = await adminAxios.delete(`/configs/remove-category-landing-page?categoryId=${id}`);
            refetch();
        } catch (ex) { }
    }
    return (
        <div className={`${className} w-100 p-5 bg-[color:var(--secondary)] border border-gray-200 rounded-lg shadow `}>
            {
                modal && <AddLandingPageCategoriesModal refetch={refetch} show={modal} closeModal={() => setModal(false)} />
            }
            <h5 className="mb-4 text-lg font-bold">
                التصنيفات في الصفحة الرئيسية
            </h5>
            <div className="flex flex-col space-y-4">
                {
                    categories.map((e, idx) =>
                        <div key={idx} className="w-full flex justify-between items-center bg-[color:var(--secondary-select)] p-2 rounded-md">
                            <Link to={`/admin/categories/${e._id}`} className="w-full h-full">
                                <p>{e.name}</p>
                            </Link>
                ...
                {
                    categories.length == 0 && <p className="text-fade">
                        لم يتم اضافة أي تصنيفات الى الان
                    </p>
                }
            </div>
